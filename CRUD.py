...
def get_players(db:Session, skip:int = 0, limit:int = 100,
    ...
    if first_name:
        query = query.filter(models.Player.first_name.like(f'%{first_name}%'))
    if last_name:
        query = query.filter(models.Player.last_name.like(f'%{last_name}%'))
...
def get_team(db:Session, skip:int = 0, limit:int = 100,
    ...
    if team_name:
        query = query.filter(models.Team.team_name.like(f'%{team_name}%'))
    if league_id:
        query = query.filter(models.Team.league_id.like(f'%{league_id}%'))
    return query.offset(skip).limit(limit).all()
